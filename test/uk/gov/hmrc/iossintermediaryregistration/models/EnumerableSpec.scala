...
class EnumerableSpec extends AnyFreeSpec with Matchers with EitherValues with OptionValues with Enumerable.Implicits {
  ...
  ".withName" - {
    ...
    "must return the correct value for a valid name" in {
      Foo.values.foreach { value =>
        withName[Foo](value.toString).value mustEqual value
      }
    }

    "must return None for an invalid name" in {
      withName[Foo]("invalid") mustBe None
    }
  }
}
